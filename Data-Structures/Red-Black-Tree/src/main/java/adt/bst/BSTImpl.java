...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	public T[] preOrder() {
		...
		rbPreOrderRecursive(getRoot(), array);
		return array;
	}

	private void rbPreOrderRecursive(BSTNode<T> node, T[] array) {
		if (!node.isEmpty()) {
			add(array, node.getData());
			rbPreOrderRecursive((BSTNode<T>) node.getLeft(), array);
			rbPreOrderRecursive((BSTNode<T>) node.getRight(), array);
		}
	}
