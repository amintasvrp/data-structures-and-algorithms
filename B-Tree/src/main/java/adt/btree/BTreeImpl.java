...
public class BTreeImpl<T extends Comparable<T>> implements BTree<T> {
	...
	public T maximumElement() {
		BNode<T> node = maximumRecursive(getRoot());
		T result = null;
		if (node != null) {
			result = node.getElements().getLast();
		}
		return result;
	}

	private BNode<T> maximumRecursive(BNode<T> node) {
		BNode<T> result = null;
		if (!node.isEmpty()) {
			int lastElementIndex = node.getElements().indexOf(node.getElements().getLast());
			if (node.getChildren().size() - 1 == lastElementIndex + 1) {
				result = maximumRecursive(node.getChildAt(lastElementIndex + 1));
			} else {
				result = node;
			}
	...
	public BNode<T> minimum(BNode<T> node) {
		return minimumRecursive(node);
	}
	
	public T minimumElement() {
		BNode<T> node = minimumRecursive(getRoot());
		T result = null;
		if (node != null) {
			result = node.getElements().getFirst();
		}
		return result;
	}

	private BNode<T> minimumRecursive(BNode<T> node) {
		BNode<T> result = null;
		if (!node.isEmpty()) {
			if (!node.getChildren().isEmpty() && !(node.getChildAt(0).isEmpty())) {
				result = minimumRecursive(node.getChildAt(0));
			} else {
				result = node;
			}
